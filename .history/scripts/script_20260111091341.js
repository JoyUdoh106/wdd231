...
function displayCourses(filteredCourses) {
    ...
    filteredCourses.forEach(course => {
        ...
        // Add a general class and a specific class if completed
        courseCard.classList.add('course-card');
        if (course.completed) {
            courseCard.classList.add('completed');
        }

        courseCard.innerHTML = <strong>${course.subject} ${course.number}</strong>;
        courseDisplay.appendChild(courseCard);
    });

    // 3. Update Total Credits using reduce
    const total = filteredCourses.reduce((sum, course) => sum + course.credits, 0);
    totalCreditsDisplay.textContent = total;
}

// Event Listeners for your navigation buttons
